# this script is used to analyse the results of the 2040 simulation run after the optimization for the BASE scenario
...
# Plot costs
plot_cost_breakdown(
    total_costs, "Total System Cost Breakdown by Technology 2040 (Stacked)"
)
plot_cost_breakdown(
    dutch_costs, "Dutch System Cost Breakdown by Technology 2040(Stacked)"
)
...
plot_load_curve(total_time_series, "Total System Load Curve by Technology 2040")
plot_load_curve(dutch_time_series, "Dutch Load Curve by Technology 2040")
...
#! 3. Plot network with line loading
...
# Function to calculate costs and deltas
def get_costs_and_deltas(network_files):
    total_costs = []
    for network_file in network_files:
        network = pypsa.Network(network_file)  # Load the network
        total_costs.append(
            round(network.objective)
        )  # Get the objective value and round it
    # Calculate deltas compared to the base case
    delta_costs = [0] + [round(cost - total_costs[0]) for cost in total_costs[1:]]
    return total_costs, delta_costs


# Get costs and deltas for both years
total_costs_2023, delta_costs_2023 = get_costs_and_deltas(network_files_2023)
total_costs_2040, delta_costs_2040 = get_costs_and_deltas(network_files_2040)


# Plotting function
def plot_costs(scenarios, total_costs, delta_costs, year_label):
    ...
    ax2 = ax1.twinx()
    bars2 = ax2.bar(
        x + width, delta_costs, width, label="Delta Costs (€)", color="orange"
    )
    ...
    ax1.set_title(f"System Costs Comparison Across Scenarios ({year_label})")
...
# Scenarios
scenarios_2023 = ["2023BASE", "2023COL", "2023EXCL"]
scenarios_2040 = ["2040BASE", "2040COL", "2040EXCL"]

# Generate plots for 2023 and 2040
plot_costs(scenarios_2023, total_costs_2023, delta_costs_2023, "2023")
plot_costs(scenarios_2040, total_costs_2040, delta_costs_2040, "2040")
